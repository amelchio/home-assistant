...
def async_numeric_state(
    ...
    if entity is None:
        raise ConditionError("Unknown entity")

    if attribute is not None and attribute not in entity.attributes:
        raise ConditionError(f"Attribute '{attribute}' does not exist")
    ...
    if value_template is None:
        ...
        try:
            ...
            raise ConditionError(f"Template error: {ex}")
    ...
    try:
        ...
        raise ConditionError(f"Value '{value}' cannot be processed as a number")
        ...
        if isinstance(below, str):
            ...
            if not below_entity or below_entity.state in (
                ...
                raise ConditionError(
                    f"The below entity '{below_entity}' is not available"
                )
    ...
    if above is not None:
        if isinstance(above, str):
            ...
            if not above_entity or above_entity.state in (
                ...
                raise ConditionError(
                    f"The above entity '{above_entity}' is not available"
                )
